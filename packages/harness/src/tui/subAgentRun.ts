/**
 * Sub-agent run tracking — pure data + event ingestion, no rendering.
 *
 * Every host that dispatches sub-agents (TUI, WS, RPC) needs the same shape:
 * a per-run summary of activity and a transcript for the detail view. This
 * module owns both and lets agents subscribe per-run.
 */
...
export interface SubAgentRun {
  id: string;
  agentName: string;
  /** Optional color hint hosts use to badge runs (`'#a64ac9'` etc.). */
  agentColor?: string;
  task: string;
...
/**
 * Append-only store of in-flight + completed sub-agent runs. Mirrors the
 * "events drive a snapshot you observe" pattern the main transcript uses,
 * but scoped per run so the host UI can subscribe to one run at a time.
 */
export class SubAgentRunStore {
  private readonly runs = new Map<string, SubAgentRun>();
  private readonly perRunListeners = new Map<string, Set<SubAgentRunListener>>();
  ...
  /** Apply a runtime event from the sub-agent's bus to its run. */
