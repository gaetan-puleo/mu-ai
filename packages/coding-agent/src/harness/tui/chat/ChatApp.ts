...
import { titleFallback } from '../../session';
...
import { errMsg, audio, type ContentPart, image, type Message } from 'mu-core';
import {
  box,
  column,
  type Color,
...
import { fgToAnsi, styleToAnsi, type Theme, ThemeProvider, themesByName } from './theme';
import {
  entryComponent,
  formatToolArgs,
  stickyHeader,
...
import { sidePanel, type PanelItem, type PanelSection } from './sidePanel';
import { MU_LOGO, muGradientColor, mixHex } from './muLogo';
...
export interface ChatHost {
  ...
  /**
   * Subscribe to host-pushed side-panel sections (e.g. arya pushing tasks /
   * deployments / custom key-value over the wire). Optional — the in-process host
   * omits it, so only the local FILES/ACTIVITY sections show. A remote host emits
   * the sections its config declares. Rendered after the local sections.
   */
  subscribePanel?(listener: (sections: PanelSection[]) => void): () => void;
  banner?: string;
  minimal?: boolean;
  /** Host display label (e.g. "mu 0.36.0" or "arya 0.6.0"), shown in the panel footer. */
  version?: string;
...
export class ChatApp {
  ...
  private attachSeq = 0;
  private panelClosed = false;
  private hostSections: PanelSection[] = [];
  private unsubscribePanel: (() => void) | undefined;
  ...
  constructor(private readonly host: ChatHost) {
    ...
    this.tui.addGlobalKeybinding({ chord: { key: 'b', ctrl: true }, handler: () => this.togglePanel() });
    ...
    this.unsubscribePanel = this.host.subscribePanel?.((sections) => {
    ...
    const unsub = run.session.subscribe((event) => {
      switch (event.type) {
        ...
        case 'turn_end':
          ...
          break;
        case 'error':
          if (!record.cancelled) handle.fail(errMsg(event.error));
          this.activeRuns.delete(record);
          unsub();
          this.runUnsubs.delete(unsub);
          break;
  ...
  private handleEvent(event: AgentSessionEvent): void {
    ...
    switch (event.type) {
      ...
      case 'usage':
        ...
        break;
      case 'error': {
        const message = errMsg(event.error);
  ...
  private async exportContext(args: string): Promise<void> {
    ...
    try {
      ...
      this.showError(`Failed to export: ${errMsg(error)}`);
    }
    this.tui.requestRender();
  }
  ...
  private dock(): Component {
    return column([...this.inputGroup(), this.statusBar(), { render: (s) => s.text(0, 0, '') }]);
  }
  ...
  private bannerBlock(): Component {
    ...
    const top = '#1e40af' as Color;
    const bottom = '#7dd3fc' as Color;
    ...
    return {
      render: (s) => {
        ...
        for (let i = 0; i < n; i++) {
          const t = n > 1 ? i / (n - 1) : 0;
          const color = mixHex(top, bottom, t);
          s.text(x, i, `${styleToAnsi({ fg: color, bold: true })}${lines[i]}${RESET}`);
        }
        s.text(0, n, '');
      },
    };
  ...
  private logoBlock(): Component {
    const theme = this.theme();
    const rows = MU_LOGO.length;
    const blockW = MU_LOGO.reduce((max, line) => Math.max(max, visibleWidth(line)), 0);
    const green = theme.colors.success;
    const teal = '#2dd4bf' as Color;
    return {
      render: (s) => {
        if (s.width <= 0) return;
        const x = Math.max(0, Math.floor((s.width - blockW) / 2));
        for (let i = 0; i < rows; i++) {
          ...
          const color = muGradientColor(green, teal, t);
          s.text(x, i, `${styleToAnsi({ fg: color })}${MU_LOGO[i]}${RESET}`);
        }
      },
    };
  }
  ...
  private panelVisible(width: number): boolean {
    return !this.panelClosed && width >= PANEL_MIN_WIDTH;
  }

  private refreshSessionTitle(): void {
    const id = this.session?.id;
    if (!id) return;
    void this.host.listSessions().then((records) => {
      if (this.session?.id !== id) return;
      const title = records.find((r) => r.id === id)?.title ?? '';
      if (title !== this.sessionTitle) {
  ...
  // The refined title lands asynchronously (title-model turn); refresh once more
  // shortly after the turn starts so the sidebar settles on the real title.
  private scheduleTitleRefresh(): void {
    clearTimeout(this.titleTimer);
    const id = this.session?.id;
    this.titleTimer = setTimeout(() => {
      if (this.session?.id === id) this.refreshSessionTitle();
    }, 1500);
  }
  ...
  private sessionTitleText(): string {
    ...
    return text ? titleFallback(text) : 'New session';
  }

  private contextSection(): PanelSection {
    if (!this.ctxTotal) {
      return { title: 'CONTEXT', items: [{ label: 'no usage yet', dim: true }] };
    }
    const pct = Math.min(100, Math.round((this.ctxUsed / this.ctxTotal) * 100));
    return {
      title: 'CONTEXT',
      items: [
        { label: `${this.ctxUsed.toLocaleString('en-US')} tokens`, dim: true },
        { label: `${pct}% used`, dim: true },
      ],
    };
  }

  private panelSections(): PanelSection[] {
    return [this.contextSection(), ...this.hostSections];
  }

  private root(): Component {
    const focused = this.focusedSub();
    const showStart = this.transcript.entries.length === 0 && !focused;
    const spacer: Component = { render: () => {} };
    const gap = (n: number): Component => ({
      render: (s) => {
        if (n > 0 && s.height >= n) s.text(0, n - 1, '');
      },
    });
    const inner = focused ? this.subAgentView(focused) : showStart
      // Start screen: mu logo (or host banner) above a centered, width-limited
      // input; status pinned at the bottom.
      ? column([
        flex(spacer),
        this.banner !== undefined ? this.bannerBlock() : this.logoBlock(),
        gap(2),
    ...
    return {
      render: (s) => {
        const theme = this.theme();
        s.fill({ x: 0, y: 0, width: s.width, height: s.height }, theme.colors.background);
        if (s.width <= 2) {
          inner.render(s);
          return;
        }
        ...
        if (showPanel) {
          s.child(sidePanel(this.panelSections(), theme, this.sessionTitleText(), this.host.version), {
            x: s.width - PANEL_WIDTH,
            y: 0,
            width: PANEL_WIDTH,
            height: s.height,
          });
