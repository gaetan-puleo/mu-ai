...
export interface PendingApproval {
  ...
  input: unknown;
  agent?: string;
  /** Session that triggered the approval, when known — lets the channel route the
   * prompt to the owning session instead of guessing from a global. */
  sessionId?: string;
}
...
export const createApprovalManager = (options: ApprovalManagerOptions = {}): ApprovalManager => {
  ...
  const request = (id: string, name: string, input: unknown, agent: string | undefined, sessionId?: string): Promise<boolean> =>
    new Promise<boolean>((resolve) => {
      const req: PendingApproval = { id, name, input, agent, sessionId };
  ...
  const hooksFor: ApprovalManager['hooksFor'] = ({ decide, agent }) => ({
    beforeToolCall: async (call) => {
      ...
      const allow = await request(newId(), call.name, call.input, agentName, call.sessionId);
