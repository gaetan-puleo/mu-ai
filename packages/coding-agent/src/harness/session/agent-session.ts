...
import type { AgentSession, AgentSessionEvent, AssembledRequest, TurnOptions } from './types';
...
export const createAgentSession = (config: AgentSessionConfig): AgentSession => {
  ...
  const decorated = tools.map((tool) => withHooks(tool, hooks, id));
    ...
    try {
      for await (
        const ev of provider.stream({
          model: config.model,
          tools: [],
          // Summarization is mechanical: never pay for reasoning on it.
          chatTemplateKwargs: { ...(config.chatTemplateKwargs ?? {}), enable_thinking: false },
          messages: [
            {
              role: 'system',
              content: [{
  ...
  const send = async (input: string | ContentPart[], opts?: TurnOptions): Promise<void> => {
