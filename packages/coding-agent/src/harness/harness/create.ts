...
import { createAgentRegistry, grantArg, loadAgents, toolDecision, toolNames, variantToChatTemplateKwargs } from '../agents';
import {
  createAgentsCommand,
...
export const createHarness = async (options: HarnessOptions): Promise<Harness> => {
      ...
      ? approvals.manager.hooksFor({
        decide: (call) => {
          ...
          const decision = approvals.decide
            ? approvals.decide(agent, call)
            : toolDecision(agent, call.name, grantArg(call.name, call.input));
          // A `bashReadOnly` agent's `bash: 'allow'` grant only covers read-only
          // commands. Anything the guard rejects is downgraded to `ask` so an
          // unattended explorer can't run a mutating command on the allow grant.
          if (decision === 'allow' && call.name === 'bash' && agent.bashReadOnly && approvals.bashGuard) {
            return approvals.bashGuard(call.input) ? 'allow' : 'ask';
          }
          return decision;
        },
        agent: () => getAgent()?.name,
  ...
  const persona = (agent: Agent, opts: { tools?: Tool[]; hooks?: AgentSessionHooks; model?: string }): AgentSession =>
    createAgentSession({
      ...
      chatTemplateKwargs: variantToChatTemplateKwargs(agent.variant),
