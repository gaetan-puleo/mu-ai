...
import type { ContentPart, Provider, Tool } from 'mu-core';
import type { Agent } from '../agents';
import { createApprovalManager, type PendingApproval } from '../permissions';
...
const fakeBash = (ran: string[]): Tool => ({
  name: 'bash',
  description: 'fake bash',
  parameters: { type: 'object', properties: { cmd: { type: 'string' } }, required: ['cmd'] },
  async run(input) {
    ran.push((input as { cmd?: string }).cmd ?? '');
    return [{ type: 'text', text: 'ran' }];
  },
});

test('bashReadOnly: an allow-granted bash command is downgraded to ask when the guard rejects it', async () => {
  ...
  const { harness, cleanup } = await makeHarness({
    providers: {
      local: scripted([
        [{ type: 'tool_call', id: '1', name: 'bash', input: { cmd: 'rm -rf /' } }],
    ...
    approvals: {
      ...
      bashGuard: (i) => (i as { cmd?: string }).cmd === 'ls',
    },
  });
  ...
  expect(asked[0].name).toBe('bash');
  expect(ran).toEqual([]);
  await cleanup();
});

test('bashReadOnly: a read-only bash command passes the guard and runs without asking', async () => {
  const manager = createApprovalManager();
  const explorer: Agent = {
    name: 'explorer',
    description: '',
  ...
  const { harness, cleanup } = await makeHarness({
    providers: {
      local: scripted([
        [{ type: 'tool_call', id: '1', name: 'bash', input: { cmd: 'ls' } }],
        [{ type: 'text', text: 'after' }],
      ]),
    },
    agents: [explorer],
