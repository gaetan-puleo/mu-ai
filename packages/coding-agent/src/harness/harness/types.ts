...
export type HarnessOptions =
  ...
  & {
    ...
    approvals?: {
      ...
      /** Read-only classifier for `bashReadOnly` agents: returns true when the bash
       * input is safe to auto-allow. Injected by the consumer (e.g. coding-agent's
       * `isReadOnlyBash`) so the harness stays free of bash specifics. */
      bashGuard?: (input: unknown) => boolean;
    };
  };
