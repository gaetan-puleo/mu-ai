...
Deno.test('an opaque box background erases content behind it (no bleed-through)', () => {
  ...
  assertEquals(getCell(buf, 0, 0).grapheme, 'h');
  assertEquals(getCell(buf, 4, 0).grapheme, ' ');
  assertEquals(getCell(buf, 4, 0).style.bg, { r: 0x12, g: 0x34, b: 0x56, a: 1, intent: 'rgb' });
});

Deno.test('a translucent box background tints content instead of erasing it', () => {
  ...
  assertEquals(getCell(buf, 4, 0).grapheme, 'X');
});
