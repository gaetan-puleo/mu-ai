...
describe('new frontmatter schema', () => {
  it('parses id + structured tools map', () => {
    const path = write(
      'b.md',
      `---
id: build
...
prompt body`,
    ...
    const def = loadAgentFile(path, 'fallback');
    expect(def).not.toBeNull();
    if (!def) return;
    ...
    expect(def.permissions?.bash).toEqual({ 'git *': 'allow', '*': 'ask' });
    expect(def.permissions?.read).toBe('allow');
  });

  it('falls back to filename stem when no id and no name', () => {
    const path = write(
      'inferred.md',
      `---
description: x
...
prompt`,
    ...
    const def = loadAgentFile(path, 'inferred');
    expect(def?.name).toBe('inferred');
  });

  it('drops deny entries from allowList', () => {
    const path = write(
      'c.md',
      `---
id: c
...
prompt`,
    ...
    const def = loadAgentFile(path, 'c');
    expect(def?.tools).toContain('read');
    expect(def?.tools).not.toContain('write');
    expect(def?.permissions?.write).toBe('deny');
  });

  it('legacy comma-list form keeps working', () => {
    const path = write(
      'legacy.md',
      `---
name: legacy
...
prompt`,
    ...
    const def = loadAgentFile(path, 'legacy');
