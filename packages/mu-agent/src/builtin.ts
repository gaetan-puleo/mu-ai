...
export const DEFAULT_PRIMARY_AGENTS: AgentDefinition[] = [
  {
    ...
    tools: ['bash', 'read_file', 'write_file', 'edit_file', 'subagent', 'subagent_parallel'],
    color: '#3498db',
    systemPrompt: [
      'You are the **build** agent. You can read & edit files, run shell commands, and dispatch subagents.',
      '',
      '- Prefer `edit_file` for surgical changes; use `write_file` only for new files or full rewrites.',
